...
// EitherTraverseRecordG transforms a map by applying a function that returns an Either to each value.
...
// EitherTraverseRecord transforms a map by applying a function that returns an Either to each value.
// If any value produces a Left, the entire result is that Left (short-circuits).
// Otherwise, returns Right containing the map of all Right values.
//
// Example:
...
func EitherTraverseRecord[K comparable, E, A, B any](f EitherKleisli[E, A, B]) EitherKleisli[E, map[K]A, map[K]B] {
	return EitherTraverseRecordG[map[K]A, map[K]B](f)
}

// EitherTraverseRecordWithIndexG transforms a map by applying an indexed function that returns an Either.
...
// EitherTraverseRecordWithIndex transforms a map by applying an indexed function that returns an Either.
// The function receives both the key and the value.
// If any value produces a Left, the entire result is that Left (short-circuits).
//
// Example:
...
func EitherTraverseRecordWithIndex[K comparable, E, A, B any](f func(K, A) Either[E, B]) EitherKleisli[E, map[K]A, map[K]B] {
	return EitherTraverseRecordWithIndexG[map[K]A, map[K]B](f)
}

//go:inline
func EitherSequenceRecordG[GA ~map[K]A, GOA ~map[K]Either[E, A], K comparable, E, A any](ma GOA) Either[E, GA] {
	return EitherTraverseRecordG[GOA, GA](F.Identity[Either[E, A]])(ma)
}

// EitherSequenceRecord converts a map of Either values into an Either of a map.
...
func EitherSequenceRecord[K comparable, E, A any](ma map[K]Either[E, A]) Either[E, map[K]A] {
	return EitherSequenceRecordG[map[K]A](ma)
}

func eitherUpsertAtReadWrite[M ~map[K]V, K comparable, V any](r M, k K, v V) M {
	r[k] = v
	return r
}

// EitherCompactRecordG discards all Left values and keeps only the Right values.
// The G suffix indicates support for generic map types.
//
// Example:
//
...
func EitherCompactRecordG[M1 ~map[K]Either[E, A], M2 ~map[K]A, K comparable, E, A any](m M1) M2 {
	...
	return RR.ReduceWithIndex(m, func(key K, _ M2, value Either[E, A]) M2 {
		return EitherMonadFold(value, onLeft, func(v A) M2 {
			return eitherUpsertAtReadWrite(out, key, v)
		})
	}, out)
}
...
func EitherCompactRecord[K comparable, E, A any](m map[K]Either[E, A]) map[K]A {
	return EitherCompactRecordG[map[K]Either[E, A], map[K]A](m)
}

func EitherTraversableRecord[K comparable, E, A, B any]() EitherTraversable[E, A, B, map[K]A, map[K]B] {
	return EitherTraverseRecordG[map[K]A, map[K]B, K, E, A, B]
}
