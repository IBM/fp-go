...
// EitherMonadExtend applies a function to an Either value, where the function receives the entire Either as input.
...
func EitherMonadExtend[E, A, B any](fa Either[E, A], f func(Either[E, A]) B) Either[E, B] {
	...
	return EitherOf[E](f(fa))
}

// EitherExtend is the curried version of [EitherMonadExtend].
...
func EitherExtend[E, A, B any](f func(Either[E, A]) B) EitherOperator[E, A, B] {
	return F.Bind2nd(EitherMonadExtend[E, A, B], f)
}
