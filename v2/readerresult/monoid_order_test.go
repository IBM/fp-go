...
func orderTagged(tag string) ReaderResult[string, string] {
	return func(env string) result.Result[string] {
		return result.Of(env + tag)
	}
}
...
func TestApplicativeMonoidOrder(t *testing.T) {
	...
	t.Run("combines the results in argument order", func(t *testing.T) {
		assert.Equal(t, result.Of("x:ax:b"), m.Concat(orderTagged(":a"), orderTagged(":b"))("x"))
	})

	t.Run("empty yields the empty value", func(t *testing.T) {
		assert.Equal(t, result.Of(""), m.Empty()("x"))
	})

	t.Run("is associative", func(t *testing.T) {
		a, b, c := orderTagged(":a"), orderTagged(":b"), orderTagged(":c")
		assert.Equal(t, result.Of("x:ax:bx:c"), m.Concat(m.Concat(a, b), c)("x"))
		assert.Equal(t, result.Of("x:ax:bx:c"), m.Concat(a, m.Concat(b, c))("x"))
	})
	...
	t.Run("reports the error of the first argument when both fail", func(t *testing.T) {
		...
		assert.Equal(t, result.Left[string](errOrderA), res)
	})
}
...
func TestAlternativeMonoidOrder(t *testing.T) {
	...
	t.Run("falls back to the success", func(t *testing.T) {
		assert.Equal(t, result.Of("x:b"), m.Concat(Left[string, string](errOrderA), orderTagged(":b"))("x"))
		assert.Equal(t, result.Of("x:a"), m.Concat(orderTagged(":a"), Left[string, string](errOrderB))("x"))
	})

	t.Run("reports the error of the second when both fail", func(t *testing.T) {
		res := m.Concat(Left[string, string](errOrderA), Left[string, string](errOrderB))("x")
		assert.Equal(t, result.Left[string](errOrderB), res)
	})
}
...
func TestAltMonoidOrder(t *testing.T) {
	...
	t.Run("prefers the first success", func(t *testing.T) {
		assert.Equal(t, result.Of("x:a"), m.Concat(orderTagged(":a"), orderTagged(":b"))("x"))
	})
	...
	t.Run("empty is the zero value and an identity", func(t *testing.T) {
		assert.Equal(t, result.Left[string](errOrderEmpty), m.Empty()("x"))
		assert.Equal(t, result.Of("x:a"), m.Concat(m.Empty(), orderTagged(":a"))("x"))
		assert.Equal(t, result.Of("x:a"), m.Concat(orderTagged(":a"), m.Empty())("x"))
	})
}
