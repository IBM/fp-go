...
// exVarFetchLogged reads the config, performs IO and reports what it fetched.
func exVarFetchLogged(key string) func(exConfig) func() (string, error) {
	return func(cfg exConfig) func() (string, error) {
		return func() (string, error) {
			...
			fmt.Println("fetched:", v)
			return v, nil
		}
	}
}

var exVarTable = exConfig{Multiplier: 3, Table: map[string]string{"host": "localhost"}}

// ---------------------------------------------------------------------------
// ReaderIOResult level
// ---------------------------------------------------------------------------

// ExampleMonadChainFirstI demonstrates running a full-stack idiomatic function
// for its effect while keeping the original value.
func ExampleMonadChainFirstI() {
	fmt.Println(exRun(exVarTable, MonadChainFirstI(Of[exConfig]("host"), exVarFetchLogged)))
...
// ExampleMonadTapI demonstrates the alias of MonadChainFirstI.
func ExampleMonadTapI() {
	fmt.Println(exRun(exVarTable, MonadTapI(Of[exConfig]("host"), exVarFetchLogged)))
...
func ExampleTapI() {
	fmt.Println(exRun(exVarTable, F.Pipe1(Of[exConfig]("host"), TapI(exVarFetchLogged))))
	fmt.Println(exRun(exVarTable, F.Pipe1(Of[exConfig]("port"), TapI(exVarFetchLogged))))
...
// ExampleChainFirstEitherIK demonstrates the Either-named alias of ChainFirstResultIK.
...
// ExampleTapResultIK demonstrates that a failed validation fails the
// computation, while a successful one leaves the value untouched.
func ExampleTapResultIK() {
	fmt.Println(exRun(exConfig{}, F.Pipe1(Of[exConfig]("42"), TapResultIK[exConfig](exVarValidate))))
	fmt.Println(exRun(exConfig{}, F.Pipe1(Of[exConfig]("nope"), TapResultIK[exConfig](exVarValidate))))
