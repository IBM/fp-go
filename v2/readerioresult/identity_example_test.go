...
// with the environment unchanged and performs no I/O.
func ExampleIdentity() {
	id := Identity[idExConfig]()

	fmt.Println(id(idExConfig{Host: "localhost", Port: 8080})())
	// Output:
	// Right[readerioresult.idExConfig]({localhost 8080})
}
...
func ExampleIdentity_projection() {
	...
	fmt.Println(viaIdentity(cfg)(), viaAsks(cfg)())
	// Output:
...
func ExampleIdentity_local() {
	...
	fmt.Println(viaLocal(wide)(), viaAsks(wide)())
	// Output:
	// Right[readerioresult.idExConfig]({localhost 8080}) Right[readerioresult.idExConfig]({localhost 8080})
}
