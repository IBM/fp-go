...
// AskValue creates a ReaderIO that reads a typed value stored in the context under key.
//
// The value is retrieved with [context.Context.Value] and type-asserted to V.
// The computation yields Some(v) if the key is present and holds a V,
...
//
// Returns:
//   - A ReaderIO[option.Option[V]] that yields the value, if present
//
// Example:
//
...
//
//	getUser := readerio.AskValue[User](userKey)
...
func AskValue[V, K any](key K) ReaderIO[option.Option[V]] {
	return FromReader(IC.AskValue[V](key))
}

// WithValue runs a ReaderIO computation with an additional key → value
...
//
// Type Parameters:
//   - A: The value type of the ReaderIO (must be given explicitly)
//   - K: The type of the key (inferred from the argument)
...
//	    readerio.Map(O.GetOrElse(F.Constant("anonymous"))),
//	)
//
//	result := F.Pipe1(
//	    greet,
//	    readerio.WithValue[string](userKey, "Alice"),
//	)
//	result(context.Background())() // "Alice"
//
// See Also:
