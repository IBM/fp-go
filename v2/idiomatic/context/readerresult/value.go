...
// AskValue creates a ReaderResult that reads a typed value stored in the context under key.
//
...
//
// Returns:
//   - A ReaderResult[Option[V]] that yields the value, if present
//
// Example:
//
...
//
//	getUser := readerresult.AskValue[User](userKey)
//	user, err := getUser(ctx) // (Some(User{...}), nil) or (None, nil)
//
// See Also:
//   - WithValue: Runs a computation with a value added to the context
//   - Asks: Derives a value from the context
func AskValue[V, K any](key K) ReaderResult[Option[V]] {
	return Asks(IC.AskValue[V](key))
}

// WithValue runs a ReaderResult computation with an additional key → value
...
//
// Type Parameters:
//   - A: The value type of the ReaderResult (must be given explicitly)
//   - K: The type of the key (inferred from the argument)
...
//	    readerresult.Map(O.GetOrElse(F.Constant("anonymous"))),
//	)
//
//	result := F.Pipe1(
//	    greet,
//	    readerresult.WithValue[string](userKey, "Alice"),
//	)
//	name, err := result(context.Background()) // ("Alice", nil)
//
// See Also:
...
func WithValue[A, K, V any](key K, value V) Operator[A, A] {
	return Local[A](func(ctx context.Context) (context.Context, context.CancelFunc) {
		return IC.WithValue[V](key)(value)(ctx), noop
	})
}
