...
// orderTagged returns a reader either that appends tag to its environment
func orderTagged(tag string) ReaderEither[string, error, string] {
	return func(env string) either.Either[error, string] {
		return either.Right[error](env + tag)
	}
}

func TestApplicativeMonoidOrder(t *testing.T) {
	m := ApplicativeMonoid[string, error](orderMonoid)
...
func TestAltMonoidOrder(t *testing.T) {
	...
	t.Run("prefers the first success", func(t *testing.T) {
		assert.Equal(t, either.Right[error]("x:a"), m.Concat(orderTagged(":a"), orderTagged(":b"))("x"))
	})
