...
type (
	// Option is an alias for option.Option[A].
	Option[A any] = option.Option[A]

	// Reader is an alias for reader.Reader[R, A].
	Reader[R, A any] = reader.Reader[R, A]

	// ReaderOption is an alias for readeroption.ReaderOption[R, A].
	// It expands directly to reader.Reader[R, option.Option[A]] to avoid
	// a multi-level cross-package alias chain that causes a Go compiler deadlock
	// (see https://github.com/golang/go/issues/65711).
	ReaderOption[R, A any] = reader.Reader[R, option.Option[A]]
)
