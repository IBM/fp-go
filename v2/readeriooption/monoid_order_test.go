...
func orderLogged(log *[]string, tag string) ReaderIOOption[string, string] {
	return func(env string) func() option.Option[string] {
...
func orderTagged(tag string) ReaderIOOption[string, string] {
	var log []string
	return orderLogged(&log, tag)
}
...
func TestAltMonoidOrder(t *testing.T) {
	...
	t.Run("does not run the second effect when the first succeeds", func(t *testing.T) {
		var log []string
		assert.Equal(t, option.Some("x:a"), m.Concat(orderLogged(&log, ":a"), orderLogged(&log, ":b"))("x")())
